...
assert.match(route, /PBN-documented company leadership/, "founder proof must not overstate an exact start year");
assert.match(route, /The method RN Studio proposes making visible/, "unobserved decision method must remain proposed strategy");
...
assert.match(route, /Skip to the strategic opportunity/, "the long-form experience needs a keyboard bypass");
...
assert.match(route, /illustrative editorial framing—not attributed advice from Mark/, "sample post must distinguish editorial hypothesis from Mark's advice");
...
assert.match(corrections, /reply to the email or direct message that delivered the link/i, "corrections must return to a private reply channel");
